...
function leave (e) {
    if (!e.detail.pathInfo.finalRequestPath.includes("/admin/jobs/workers")) { // user leaves page
...
function initDetails() {
    ...
    document.querySelectorAll("[data-js-worker-jobTypes]").forEach( elem => {
        ...
        if (Arrower.Jobs.Worker[worker]) {
            ...
            elem.nextElementSibling.nextElementSibling.nextElementSibling.classList.remove('hidden');
        }

        elem.addEventListener("click", () =>  {
            elem.nextElementSibling.classList.toggle('hidden');
            elem.nextElementSibling.nextElementSibling.classList.toggle('hidden');
            elem.nextElementSibling.nextElementSibling.nextElementSibling.classList.toggle('hidden');
