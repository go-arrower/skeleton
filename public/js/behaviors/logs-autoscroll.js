...
document.querySelectorAll("[data-js-logs-autoscroll]").forEach(elem => {
    ...
    elem.addEventListener('keydown', e => {
        if (e.code==='PageDown' || e.code==='End' || e.code==='ArrowDown') {
            if (!Arrower.Log.auto && (elem.scrollTop + elem.clientHeight >= elem.scrollHeight)) {
                Arrower.Log.startLive();
            }
        }
    })
    ...
    Arrower.Log.autoScrollerRenderer = window.setInterval(function() {
        if (Arrower.Log.auto) {
            Arrower.Log.autoScrollerButton.innerHTML = Arrower.Log.autoScrollerButtonLive;
...
function leave (e) {
    if (!e.detail.pathInfo.finalRequestPath.startsWith("/admin/logs")) { // user leaves page
